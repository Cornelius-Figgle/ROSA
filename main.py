...
def startup():
    ...
    if isOn_RPi == True: 
        ...
        gpio_loc['active'] = int(config.get('LEDs', 'active'))
        gpio_loc['listen'] = int(config.get('LEDs', 'listening'))
        gpio_loc['process'] = int(config.get('LEDs', 'processing'))
        gpio_loc['out'] = int(config.get('LEDs', 'speaking'))
        gpio_loc['off_sw'] = int( config.get('switch', 'shutdown'))
        ...
        GPIO.add_event_detect(gpio_loc['off_sw'], GPIO.RISING, callback=os.system('shutdown -h now'))
